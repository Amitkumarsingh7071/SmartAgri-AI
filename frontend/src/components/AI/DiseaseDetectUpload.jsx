  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start text-left">
      ...
      <div className="lg:col-span-5 glass-panel p-6 rounded-3xl border border-gray-200/50 dark:border-gray-800/30">
        ...
        <p className="text-xs text-gray-500 mb-6">{t('disease.subtitle')}</p>
        ...
        <form onSubmit={handleUpload} className="space-y-4">
          ...
          <div className="w-full h-56 border-2 border-dashed border-gray-300 dark:border-gray-700 hover:border-green-500 rounded-2xl overflow-hidden flex flex-col justify-center items-center cursor-pointer transition-colors relative bg-gray-50/20 dark:bg-gray-800/5">
            {previewUrl ? (
              ...
              <div className="flex flex-col items-center p-6 text-center">
                ...
                <span className="text-xs font-semibold text-gray-600 dark:text-gray-400">{t('disease.dragClick')}</span>
                <span className="text-[10px] text-gray-400 mt-1">{t('disease.supportedCrops')}</span>
              </div>
            )}
            <input
              type="file"
          ...
          >
            <span>{loading ? t('disease.analyzingLeaf') : t('disease.runDiagnostics')}</span>
          </button>
        </form>
      </div>
        ...
        {result ? (
          <div className="glass-panel p-6 rounded-3xl border border-gray-200/50 dark:border-gray-800/30 space-y-5 animate-in fade-in duration-300">
            ...
            <div className="flex justify-between items-start border-b border-gray-100 dark:border-gray-800/50 pb-4">
              <div>
                <span className="text-[9px] font-extrabold uppercase text-gray-400 block tracking-wide">{t('disease.resultHeading')}</span>
              ...
              <div className="text-right">
                <span className="text-xs text-gray-400 font-bold block">{t('disease.confidence')}</span>
            ...
            {/* Causes */}
            <div>
              <span className="text-gray-400 uppercase tracking-wide text-[9px] font-bold block">{t('disease.causes')}</span>
            ...
            {/* Preventive Measures */}
            <div className="pt-2">
              <span className="text-gray-400 uppercase tracking-wide text-[9px] font-bold block">{t('disease.preventiveMeasures')}</span>
          ...
          <div className="glass-panel p-12 rounded-3xl border border-gray-200/50 dark:border-gray-800/30 flex flex-col justify-center items-center text-center h-full min-h-[350px]">
            ...
            <h4 className="font-bold text-sm text-gray-400">{t('disease.awaitingPhoto')}</h4>
            <p className="text-[11px] text-gray-500 mt-1 max-w-[240px]">{t('disease.awaitingSubtext')}</p>
          </div>
        )}
      </div>
    </div>
